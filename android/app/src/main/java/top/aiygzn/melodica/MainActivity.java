...
public final class MainActivity extends Activity {
    ...
    @Override public void onCreate(Bundle saved) {
        ...
        Ui.text(root, "DELTA MELODICA  /  0.6", 10, Ui.ACCENT).setLetterSpacing(.12f);
        ...
        text(launch, "进入口风琴画面 → 校准 12 个按钮 → 确认半音状态 → 播放。首次倒计时 3 秒。", 13, Ui.TEXT);
    ...
    private void selectSong(int index) {
        ...
        } catch (Exception e) { refreshing = false; selected = null; toast("曲谱读取失败：" + e.getMessage()); }
    }
    private void applySong() {
        if (original == null) return;
        try { selected = settings.prepare(original, defaultPiano()); prepare(); describe(); }
        catch (Exception e) { selected = null; if (MelodicaService.instance != null) MelodicaService.instance.stop(); songInfo.setText("请修正设置：" + e.getMessage()); toast(e.getMessage()); }
    }
    private void describe() {
        ...
        if (segmentInfo != null) { try { int count = settings.segments().size(); segmentInfo.setText(count == 0 ? "演奏全曲 · 可设置片段" : "已编排 " + count + " 个片段 · 原曲 " + MelodicaService.time(original.duration)); } catch (Exception e) { segmentInfo.setText(e.getMessage()); } }
    }
    ...
    @Override protected void onDestroy() { if (previewPlayer != null) previewPlayer.close(); statusHandler.removeCallbacksAndMessages(null); super.onDestroy(); }
