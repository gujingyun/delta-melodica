...
在 0.7.0 三页界面上移植最终 32 毫秒快速切调方案（来源 `3489686f` 及其切调衔接基础），保留稳定模式。进入「设置 → 切调速度 → 快速（试用）」启用；默认仍为稳定模式，选择会保存。32 毫秒是目标空隙，并非所有手机与游戏的保证值。

松键与变音手势顺序合批，利用休止与有限气口减少累计拖拍；下一音仍等待完成回调和前台检查。没有引入 10 毫秒探测方案。详情见 [整合验证](../docs/安卓快速切调整合-20260919.md)。
