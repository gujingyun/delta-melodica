...
从 v0.1 升级后保留曲库，但需要重新完成 12 点校准；已有 v0.2／v0.3 的曲库和 12 点校准可继续使用。可以先在「本地测试键盘」检查触摸和选中状态；进入真实游戏后需重新校准。更换分辨率、屏幕方向或游戏键位后也应重新校准。
...
构建脚本先执行 `clean testDebugUnitTest assembleDebug lintDebug`，成功后复制到仓库根目录 `dist\三角洲口风琴_安卓_v0.4预览版.apk`，并输出 SHA-256。使用完整清理构建，避免增量打包残留旧 dex。

22 项单元测试覆盖简谱、MIDI、旋律整理、音高映射、暂停续播、倒计时取消、变音状态、顺序点击批次、提前切换的时钟截止点，以及线上目录校验、搜索、HTTPS 地址、下载大小与哈希、取消及本地去重保存。JSON 测试依赖只在本机测试运行，不打入 APK。
