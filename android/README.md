...
6. **倒计时和演奏期间「收起」置灰禁用**；暂停或停止并完成触摸释放后恢复可用，曲目结束也会恢复。收起后可返回主界面恢复悬浮窗。彻底退出演奏服务可点主界面「停止并关闭演奏服务」，或在系统无障碍设置中关闭。

从 v0.1 升级后保留曲库，但需要重新完成 12 点校准；已有 v0.2 的 12 点校准可继续使用。可以先在「本地测试键盘」检查触摸和选中状态；进入真实游戏后需重新校准。更换分辨率、屏幕方向或游戏键位后也应重新校准。
...
窗口包名仅用于前台确认。不读取窗口文字、不截图，不进行游戏进程读取或修改。v0.3 增加联网权限，仅在线上曲库读取目录和下载 MIDI，不上传本地曲谱、设置或窗口信息。曲库和校准保存在应用私有目录；卸载会移除。测试键盘校准不会用于游戏窗口，返回主界面也不会触发测试音键。
...
构建脚本先执行 `clean testDebugUnitTest assembleDebug lintDebug`，成功后复制到仓库根目录 `dist\三角洲口风琴_安卓_v0.3预览版.apk`，并输出 SHA-256。使用完整清理构建，避免增量打包残留旧 dex。

19 项单元测试覆盖简谱、MIDI、旋律整理、音高映射、暂停续播、倒计时取消、变音状态、时钟保持，以及线上目录校验、搜索、HTTPS 地址、下载大小与哈希、取消及本地去重保存。JSON 测试依赖只在本机测试运行，不打入 APK。
...
15 项触摸设备回归包含真实 12 点校准、变音与播放生命周期，以及倒计时／演奏时收起禁用、结束／暂停／停止后恢复、实际点击禁用按钮不收起。线上曲库另有 5 项设备验证，覆盖官网目录和 MIDI 下载、搜索分页、取消／下载／返回主界面、重复选用、加载期间关闭页面。测试入口会清理本次创建的测试下载，保存自建界面截图；正常演奏不截图。
...
![播放期间禁用收起](docs/collapse-disabled-v03.png)
