...
旧 v0.5 等预览包与正式包签名不同，不能直接覆盖安装。先在旧包中同步曲库并在官网核对，或保留可重新导入的原始文件，再卸载旧包并安装正式包；本机设置和校准需重新配置。未同步且没有原始文件的曲谱不要先卸载。后续同签名正式版可以覆盖升级。此候选包的实测、发布材料和待确认项见 [上线准备记录](docs/上线准备-v0.6.0.md)。

2026-09-14 用户授权将安卓 app 及相关代码提交到 GitHub，源码、构建脚本、测试与文档统一整理到 `codex/android-app-source` 分支。签名密钥、加密密码、个人数据及构建产物继续保存在仓库外或忽略目录中。
...
本阶段记录时真实发信和线上账号 API 尚待部署，安卓代码及 APK 仅保留在本地；后续部署状态见本文开头，源码公开已于 2026-09-14 获得用户授权。
