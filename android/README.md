...
本地更新包为 `dist/delta-melodica-android-v0.6.2.apk`，可覆盖升级 v0.6.0／v0.6.1 正式签名安装包。官网当前下载仍为 v0.6.0。
...
5. 将悬浮窗拖到不遮挡这 12 个按钮的位置。点「播放」，按提示在游戏中将「半音」设为未选中，再点「开始演奏」；倒计时中预选首音的变音状态，3 秒后才按下音键。「暂停」保留位置；「继续」时同样先手动关闭半音，再点「开始演奏」从该处续播；「停止」回到曲首。
...
- 半音没有独立的「关闭」按钮，因此每次开始、续播都提示用户先将游戏内半音设为未选中，随后点击「开始演奏」。程序按已关闭的初态切换音区和半音；提示期间不会自动演奏，也不会读取或自动关闭游戏半音。暂停、停止、切出或手势中断会作废内部状态记录。手动调整游戏变音前请先暂停。
...
![播放前提示用户关闭半音](docs/half-prompt-v062.png)
