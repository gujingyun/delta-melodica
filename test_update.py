...
class UpdateTests(unittest.TestCase):
    def test_version_parser_accepts_optional_prefix_and_compares_numeric_parts(self):
        ...
        self.assertGreater(parse_version("0.11"), parse_version(APP_VERSION))
        self.assertLess(parse_version("0.9"), parse_version(APP_VERSION))

    def test_manifest_requires_valid_version_and_normalizes_notes(self):
        manifest = parse_update_manifest({"version": "v0.11", "notes": "  新增更新检查。  "})
        self.assertEqual(manifest, {"version": "0.11", "version_tuple": (0, 11), "notes": "新增更新检查。"})
        for payload in ({}, {"version": "latest"}, {"version": "0.11", "notes": []}):
            with self.subTest(payload=payload), self.assertRaises(ValueError):
                parse_update_manifest(payload)

    def test_fetch_manifest_decodes_json_with_timeout(self):
        response = Mock()
        response.read.return_value = json.dumps({"version": "0.11", "notes": "修复问题"}).encode("utf-8")
        ...
        self.assertEqual(manifest["version"], "0.11")
    ...
    def test_update_notice_opens_official_page_after_confirmation(self):
        ...
        app = App.__new__(App)
        app.root = object()
        manifest = {"version": "0.11", "notes": "新增检查更新。"}
